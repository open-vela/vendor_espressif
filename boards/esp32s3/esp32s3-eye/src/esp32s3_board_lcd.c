...
int board_lcd_initialize(void) {
  syslog(LOG_INFO, "LCD: board_lcd_initialize start\n");
  ...
  syslog(LOG_INFO, "LCD: backlight on, initializing SPI\n");
  ...
  syslog(LOG_INFO, "LCD: SPI ok, initializing ST7789\n");
  ...
  syslog(LOG_INFO, "LCD: ST7789 ok, setting power\n");

  g_lcd->setpower(g_lcd, CONFIG_LCD_MAXPOWER);

  syslog(LOG_INFO, "LCD: init complete\n");
